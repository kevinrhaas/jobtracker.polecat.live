...
function topics(){ return [
  ...
  { id:'getting-started', group:'user', title:'Getting started', icon:'rocket', html:`
    ...
    <h3>Quick add</h3>
    <p>Press <kbd>Q</kbd> or the wand icon in the top bar for a faster way in: type a plain
    sentence like <i>"rush social post for Membership due Friday"</i> and JobTracker picks out
    the type, client or campaign, due date and rush flag as you type, then creates the job and
    opens the full editor so you can fill in anything it didn't catch.</p>
    <h3>The dashboard</h3>
  ...
  { id:'managing-jobs', group:'user', title:'Managing jobs', icon:'edit', html:`
    ...
    <ul>
      ...
      <li><b>Attachments</b> — link deliverables and reference files to the job. Uploading a new
      file over an existing one via the upload icon keeps the old file in a per-attachment
      <b>version history</b> — open it with the clock icon to download or restore any prior
      version.</li>
    ...
    or delete one you no longer need — deletes are undoable too. Cloning never carries recurrence
    forward, so duplicating a repeating job doesn't accidentally start a second series.</p>
    <h3>Recurring jobs</h3>
    <p>For work that repeats — a weekly newsletter, a monthly report, an annual renewal — open the
    Details tab's <b>Recurring</b> section, set a due date, and turn on <b>Repeat this job</b>. Pick a
    cadence (weekly, every 2 weeks, monthly, quarterly, or annually) and how many days before the due
    date the next occurrence should be created. Once that day arrives, the next occurrence auto-spawns
    with the same type, owner, division, campaign and checklist — no need to remember to clone it
    yourself.</p>
    <h3>Dependencies</h3>
    <p>When one deliverable gates another — photography before the print ad that uses it, a brand
    refresh before the collateral built on it — open the Details tab's <b>Dependencies</b> section
    and <b>Add blocker</b> to link the job it's waiting on. A job with any open (non-terminal)
    blocker shows a <b>Blocked</b> chip in its hero and on its Board card / Jobs row; moving a
    blocked job's status asks for a quick confirmation instead of silently allowing it, the same
    pattern as an unusual status transition. The <b>Blocks</b> list is the read-only inverse — every
    job waiting on this one — so you can see the whole chain from either side. Circular
    dependencies are rejected outright.</p>
    <h3>Focus mode</h3>
    <p>Heads-down on one deliverable? Click the target icon in a job's hero (or its
    <b>Focus mode</b> action in the Jobs list) for a calm, full-screen view of just that
    job's name, status, checklist and comment thread — no tabs, no rail, no topbar. Copy
    a shareable focus link, jump to the full editor any time, or press <kbd>Esc</kbd> to leave.</p>` },
  ...
  { id:'views-filters', group:'user', title:'Views, filters & export', icon:'list', html:`
    ...
    <ul>
      ...
      <li><b>Board</b> — a Kanban board grouped by status; drag a card to change its status,
      or focus one and use the arrow keys to move around and <kbd>Shift</kbd>+<kbd>←</kbd>/<kbd>→</kbd>
      to change its status — no mouse required.</li>
    ...
    it'll be one click away next time. Manage every view from <b>Settings → Saved views</b>.</p>
    <p>Click the <b>link icon</b> next to a saved view (in the Jobs list or the View Library) to
    copy a shareable link. Opening it loads that same filter/column/sort combination as an
    unsaved working view, with a banner offering to save it into your own library — the whole
    view config rides in the link itself, nothing is uploaded anywhere.</p>
    <h3>Exporting</h3>
  ...
  { id:'reports', group:'user', title:'Reports', icon:'presentation', html:`
    <h2>Reports</h2>
    <p><a data-go="reports">Reports</a> generates a shareable summary for any date range — the
    "end of year report" you'd otherwise build by hand. Unlike <a data-go="metrics">Metrics</a>,
    which always shows the live, right-now picture, Reports is scoped to a period and compares it
    against the equivalent prior period.</p>
    <h3>Picking a period</h3>
    <p>Choose a preset (This year, Last year, This/Last quarter, This month, Last 12 months, All
    time) or <b>Custom…</b> to pick your own start and end month.</p>
    <h3>What's in it</h3>
    <ul>
      <li>KPIs — completed, created, on-time delivery, average cycle time, and rush share — each
      with a trend arrow versus the prior period of the same length.</li>
      <li>Breakdowns of completed work by type, division, client and owner; click any bar to open
      that slice in the Jobs list.</li>
      <li>A monthly completed-jobs chart across the period.</li>
    </ul>
    <h3>Sharing it</h3>
    <p>Use <b>Copy summary</b> for a quick plain-text digest to paste into an email or chat,
    <b>Export Excel</b> for the full KPI + breakdown tables as a spreadsheet, or <b>Print report</b>
    for a clean, print-friendly page (Save as PDF from the browser's print dialog works great).</p>
    <h3>Custom KPI dashboards</h3>
    <p>The <b>Custom</b> tab on <a data-go="metrics">Metrics</a> lets you build your own KPI cards —
    pick a metric (count, sum/average of a field, average cycle time, or on-time delivery %), filter
    it to exactly the status, type, division, owner or client you care about, and optionally scope it
    to a date period. Save several cards into a named dashboard, and keep as many dashboards as you
    like — a "Client X Overview" alongside your studio-wide one, for example. Everything is computed
    live from your current jobs; only the card configuration itself is saved.</p>` },
  ...
  { id:'sharing', group:'user', title:'Sharing & access', icon:'key', html:`
    ...
    <h3>Kiosk / intake links</h3>
    <p>A kiosk link (minted from Admin with the "Kiosk / intake-only" toggle) skips the app
    entirely and opens straight into a friendly, distraction-free <b>"Submit a job request"</b>
    form — no dashboard, no nav, no other jobs visible. Whoever fills it out and hits submit
    creates a real job (in the "Requested" status, tagged with a small <b>Intake</b> badge)
    directly in whichever workspace opened the link — great for a shared front-desk device
    where anyone on the team can drop off a request.</p>
  ...
  { id:'shortcuts', group:'user', title:'Tips & shortcuts', icon:'bolt', html:`
    ...
    <table class="kbd-tbl">
      ...
      <tr><td><kbd>Q</kbd></td><td>Quick add — describe a job in plain text ("rush social post for Membership due Friday") and JobTracker fills in the type, client/campaign, due date and rush flag for you</td></tr>
  ...
  { id:'architecture', group:'dev', title:'Architecture overview', icon:'layers', html:`
    ...
    <ul>
      ...
      <li><span class="mono">views/*</span> — one module per section (home, inventory, board, calendar, timeline, campaigns, metrics, documents, import, docs, admin, settings) exporting a <span class="mono">render…(view, ctx, params)</span> function.</li>
      ...
      <li><span class="mono">idb.js</span> — a tiny IndexedDB wrapper storing attachment file bytes, keyed by attachment id (see <a data-jump="storage">Storage &amp; migration</a>).</li>
    </ul>` },
  ...
  attachments:[{id,name,size,type,ts,version,mock,tags:[],by,
    versions:[{version,blobId,name,size,type,ts,by,mock}]}],
  subtasks:[{id,text,done}], milestones:[{id,name,date,done}],
  blockedBy:[jobId], approval:{state,rounds:[]}, icon,
  createdAt, updatedAt, createdBy, updatedBy }</code></pre>
    ...
    <span class="mono">statuses</span> (name, color, order, terminal, ageDays, allowedNext, wipLimit),
    ...
    <h3>Dependencies</h3>
    <p>A job's <span class="mono">blockedBy</span> is a list of other job ids it depends on; the
    inverse ("blocks") is never stored, only computed by scanning for jobs whose
    <span class="mono">blockedBy</span> includes this one — same approach as campaign membership.
    Adding a blocker is rejected if it would create a cycle (walks the candidate's own
    <span class="mono">blockedBy</span> chain looking for a path back to the job being edited).</p>
    <h3>Numbering</h3>
    <p>Job numbers start at <span class="mono">14800</span> and increment; uniqueness is enforced
    on write. The schema is versioned (<span class="mono">SCHEMA = 13</span>).</p>` },

  { id:'storage', group:'dev', title:'Storage & migration', icon:'history', html:`
    <h2>Storage &amp; migration strategy</h2>
    <h3>Today: localStorage + IndexedDB</h3>
    <p>The workspace (jobs, metadata, views, settings, audit trail) lives in
    <span class="mono">localStorage</span> as one JSON blob, re-serialized on every mutation —
    simple, synchronous, offline-friendly, at the cost of a few-MB quota (the store emits a
    <span class="mono">quota</span> event when it fills up). That blob stays text-only and
    portable for export/import because binaries live elsewhere.</p>
    <h3>Attachments</h3>
    <p>File bytes are stored in <b>IndexedDB</b> (<span class="mono">js/idb.js</span>, database
    <span class="mono">jt-files</span>), keyed by the attachment's id — a job's
    <span class="mono">attachments</span> array only ever holds metadata (name, size, type, tags,
    who added it). Turning on <b>Mock Uploads</b> (Settings → Data &amp; Privacy) skips IndexedDB
    entirely and keeps metadata only, capped by <span class="mono">maxFileMB</span>. Uploading a
    replacement file archives the outgoing one into that attachment's
    <span class="mono">versions</span> list (its own metadata + a separate IndexedDB key) so it can
    be downloaded or restored later — restoring always creates a new version rather than rewinding
    the counter, keeping the history a clean, ever-increasing timeline. The
    <a data-go="documents">Document Library</a> section lists every attachment across every job for
    search, tagging and cleanup. Because binaries live in IndexedDB rather than the exported JSON,
    a workspace export/import moves metadata but not file bytes, and <b>Reset all local data</b>
    (Settings → Data &amp; Privacy) clears both.</p>
