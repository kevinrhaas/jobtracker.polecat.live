...
export function openJob(id, ctx={}){
  ...
  function detailsTab(){
    ...
    // divisions + designers (wide, pill toggles)
    wrap.append(multiPills('Divisions', job.divisions, meta.divisions, v=>applyPatch({divisions:v})));
    wrap.append(multiPills('Designers', job.designers, peopleNames, v=>applyPatch({designers:v}), { allowAdd:true, hint:'Pick from the team or type a name and press Enter.' }));

    // icon picker
    wrap.append(iconPicker());

    // finance (collapsible, hidden in simple mode — already returned above)
    const fin = el('details',{class:'card pad fin-block'});
    fin.append(el('summary',{class:'fin-summary', html:icon('db',16)+'<span>Finance &amp; tracking</span>'}));
    const fg = el('div',{class:'def-grid', style:'margin-top:14px'});
    fg.append(
