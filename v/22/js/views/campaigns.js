...
import { isOverdue } from './shared.js';
...
export function renderCampaigns(view, ctx, params={}){
  ...
  if(!camps.length){
    view.append(el('div',{class:'empty'},[
      el('div',{class:'e-ic', html:icon('flag',30)}),
      el('h3',{text:'No campaigns yet'}),
      el('p',{text:'Group jobs that belong to the same push — a product launch, an annual event, a rebrand — and see rollup status at a glance.'}),
      el('button',{class:'btn primary', html:`${icon('plus')} New campaign`, onclick:()=>openEditor(null, ctx)}),
    ]));
    return;
  }
