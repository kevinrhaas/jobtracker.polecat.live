...
export const Access = new (class {
  ...
  grant(via, label, intake=false){ try{ localStorage.setItem(A_KEY, JSON.stringify({ grantedAt:Date.now(), via, label:label||'', intake:!!intake })); }catch{} }
  info(){ try{ return JSON.parse(localStorage.getItem(A_KEY)||'null'); }catch{ return null; } }
  revokeSelf(){ localStorage.removeItem(A_KEY); }
  // A kiosk / intake-only token — see admin.js "Kiosk / intake-only link".
  // Boots straight into the distraction-free request form (views/intake.js)
  // instead of the full app shell.
  isIntake(){ return !this.isAdmin() && !!this.info()?.intake; }
  ...
  async mintInvite({ label='', days=0, job='', intake=false }={}){
    ...
    const body = { v:1, label, iat, exp, jti, intake: !!intake };
    ...
    // A share link may deep-link to a specific job (kiosk/intake links never do).
    const hash = (!intake && job) ? `#job/${encodeURIComponent(job)}` : '';
    const link = `${location.origin}/app/?token=${encodeURIComponent(code)}${hash}`;
    this._remember({ label, iat, exp, jti, code, link, job: intake?'':job, intake:!!intake });
    return { code, link, iat, exp, label, jti, intake:!!intake };
  }
  ...
  async init(){
    ...
    if(code){
      ...
      if(await this.verifyAdminToken(code)){ await this.unlockAdmin(code); }
        ...
        if(r.ok) this.grant('token', r.payload.label||'', !!r.payload.intake);
