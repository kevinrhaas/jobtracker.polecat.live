...
const SCHEMA  = 3;                    // bump when the shape changes
...
export const Store = new (class extends Emitter{
  ...
  blankJob(patch={}){
    ...
    return {
      ...
      name: '', type: this.meta().types[0]?.name || 'Design',
      ...
      campaign:'', comments:[], attachments:[], milestones:[],
  ...
  removeView(id){ this.data.views=this.data.views.filter(v=>v.id!==id); this._persist(); this.emit('views'); this.emit('change'); }
