...
export function renderBoard(view, ctx, params){
  ...
  function build(){
    ...
    function columnEl(status, jobs, cols, colIndex){
      ...
      if(jobs.length){
        // Incremental rendering keeps huge boards fast (>1000 jobs): each
        // column materializes cards in chunks as you scroll its body — the
        // sentinel button at the bottom loads the next chunk when it comes
        // into view (or on click/Enter, which also covers keyboard users and
        // any browser without IntersectionObserver). The column-head count
        // always shows the true total; drag targets are the column body, so
        // drops work no matter how much is materialized.
        const CHUNK = 40;
        let shown = 0;
        const sentinel = el('button',{class:'bcol-more', type:'button'});
        const appendChunk = ()=>{
          jobs.slice(shown, shown+CHUNK).forEach(j=> body.insertBefore(cardEl(j, cols, colIndex), sentinel));
          shown = Math.min(jobs.length, shown+CHUNK);
          const left = jobs.length - shown;
          if(left<=0){ io?.disconnect(); sentinel.remove(); }
          else sentinel.textContent = `Show ${Math.min(CHUNK,left)} more (${left} below)`;
        };
        sentinel.addEventListener('click', appendChunk);
        body.append(sentinel);
        let io = null;
        if(typeof IntersectionObserver==='function'){
          io = new IntersectionObserver(es=>{ if(es.some(e=>e.isIntersecting)) appendChunk(); },
