...
export function renderSettings(view, ctx, params={}){
  ...
  function statusEditor(statuses){
    ...
    statuses.forEach((s,i)=>{
      ...
      wrap.append(el('div',{class:'meta-item'},[ color, name, age, term, flow, el('div',{class:'mi-btns'},[...moveBtns('statuses', i, statuses.length), del]) ]));
    });
    ...
    const add = el('form',{class:'field-row', style:'margin-top:14px', onsubmit:e=>{
      ...
      Store.addMetaValue('statuses', { name:v, color:'#8b5cf6', order, terminal:false, ageDays:7, allowedNext:[] }); ni.value='';
    }}, [ ni, el('button',{class:'btn primary', type:'submit', html:icon('plus',15)+' Add status'}) ]);

    return block('Statuses',
      'Workflow stages with a color, an aging threshold (days), and whether the stage is terminal (done). Use ↑ ↓ to set board & pill order. "Can move to" is optional — when set, moving a job somewhere else still works but asks for confirmation first.',
      [wrap, add]);
  }
  ...
  function viewRow(v, i, len, isDefault){
    ...
    return el('div',{class:'meta-item view-row'},[
      ...
      summary,
      el('div',{class:'mi-btns'},[ open, ...moveBtns2(i, len), dup, del ]),
    ]);
