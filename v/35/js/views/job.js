...
import { humanSize, attIcon, emptyBlock, attachmentHistoryNode, findSimilarJobs } from './shared.js';
  ...
  function detailsTab(){
    ...
    const statusField = ()=>{
      ...
      s.addEventListener('change', async ()=>{
        ...
        if(!Store.isTransitionAllowed(from, to)){
          const ok = await confirmDialog({ title:'Unusual status change',
            message:`Jobs don't normally move from "${from}" straight to "${to}". Change it anyway?`,
            okText:'Change anyway' });
          if(!ok){ s.value = from; return; }
        }
