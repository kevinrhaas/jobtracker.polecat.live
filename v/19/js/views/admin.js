...
export function renderAdmin(view, ctx){
  ...
  function renderLocked(){
    ...
    wrap.append(
      el('div',{class:'e-ic'}, el('span',{html:icon('shield',30)})),
      el('h3',{text:'Admin mode is locked'}),
      el('p',{text:'The admin console mints and manages the shareable access links that let people into this invite-only app. Paste your admin token to unlock it on this device.'}),
    );
  ...
  function mount(){
    ...
    const genBtn = el('button',{class:'btn primary', html:icon('link',16)+' Generate link', onclick:async()=>{
      ...
      try{
        ...
        showMintResult(inv);
        toast('Invite link created',{kind:'ok'});
    ...
    const mintCard = block('Mint an access link', 'Create a shareable link into the app. Optionally set an expiry and deep-link to one job.', [
      el('div',{class:'field-row'},[
        field('Label (optional)', labelI, 'Who / what this link is for.'),
        field('Expires in (days)', daysI, '0 = never expires.'),
      ]),
      field('Deep-link to a job (optional)', jobSel, 'The recipient lands directly on this job.'),
      el('div',{},[ genBtn ]),
      result,
    ]);
    ...
    function renderMinted(){
      ...
      minted.forEach(inv=>{
        ...
        const tr = el('tr');
        tr.append(
          el('td',{},[ el('b',{text:inv.label||'—'}), el('div',{class:'mono tiny muted', text:inv.jti||''}) ]),
