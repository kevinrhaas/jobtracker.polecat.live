...
let dashTab = 'overview';   // 'overview' | 'custom'
...
function blankWidget(){
  return { id:uuid(), title:'', icon:'chart', accent:false, metric:'count', field:'quantity',
    filters:{ status:[], type:[], division:[], owner:[], client:[], rush:false },
...
function matchWidget(j, w){
  ...
  if(f.owner?.length    && !f.owner.includes(j.owner)) return false;
...
function widgetEditor(existing){
  return new Promise(resolve=>{
    ...
    w.filters = { status:[], type:[], division:[], owner:[], client:[], rush:false, ...(w.filters||{}) };
    ...
    const body = el('div',{},[
      ...
      field('Owner', pillGroup(w, 'owner', Store.people().map(p=>p.name))),
...
export function renderMetrics(view, ctx, params){
  ...
  const head = sectionHead('Metrics', dashTab==='custom'
    ? 'Build your own KPI cards, filtered exactly how you like, saved into dashboards.'
    : 'Live status reporting — everything is computed from your current jobs.');
  const seg = el('div',{class:'tl-seg no-print', role:'group', 'aria-label':'Metrics view'});
  [['overview','Overview'],['custom','Custom']].forEach(([key,label])=>{
