...
// `onClick`, when given, makes the tile a real link into the Jobs list
// filtered to whatever it's counting (keyboard-accessible, like jobTile).
function kpi(value, label, iconName, { accent, danger, decimals=0, suffix='', trend, onClick }={}){
  const cls = 'kpi' + (danger && value>0 ? ' danger' : accent ? ' accent' : '');
  const val = el('div',{class:'k-val'});
  const props = onClick
    ? { class:cls, role:'button', tabindex:'0', 'aria-label':`${label} — view in Jobs`,
        onclick:onClick, onkeydown:e=>{ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); onClick(); } } }
    : { class:cls };
  const tile = el('div', props, [
...
// Navigate to the inventory list filtered to an exact set of job ids — used
// by the KPI tiles so what you land on always matches the number you clicked,
// even for stats (avg cycle, on-time %) with no single status/type to filter by.
function goIds(ctx, ids, label){
  window.__pendingFilter = { ids, label };
  ctx.go('inventory');
}
...
export function renderHome(view, ctx, params){
  ...
  kpis.append(
    kpi(active.length,      'Active jobs',        'layers',   { accent:true,
      onClick:()=>goIds(ctx, active.map(j=>j.id), 'Active jobs') }),
    kpi(dueWeek.length,     'Due this week',      'calendar', {
      onClick:()=>goIds(ctx, dueWeek.map(j=>j.id), 'Due this week') }),
    kpi(overdue.length,     'Overdue',            'warn',     { danger:true,
      onClick:()=>goIds(ctx, overdue.map(j=>j.id), 'Overdue') }),
    kpi(completedMo.length, 'Completed this month','check',   {
      onClick:()=>goIds(ctx, completedMo.map(j=>j.id), 'Completed this month') }),
    kpi(avgCycle,           'Avg cycle (days)',   'clock',    { decimals:1,
      onClick:()=>goIds(ctx, completed.map(j=>j.id), 'Avg cycle (days)') }),
    kpi(onTimePct,          'On-time delivery',   'target',   { suffix:'%',
      onClick:()=>goIds(ctx, withDue.map(j=>j.id), 'On-time delivery') }),
  );
  view.append(kpis);
