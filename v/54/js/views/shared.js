...
//            rush:bool, mine:bool, campaign, overdue:bool, ids:[] }
export function applyFilters(jobs, filters={}, actor=''){
  ...
  if(f.mine && actor)    out=out.filter(j=>[j.assignee,j.requester].includes(actor));
  if(f.ids)              out=out.filter(j=>f.ids.includes(j.id));
  return out;
}
...
// ---- undo toast -----------------------------------------------------------
// The safety net for destructive actions that live OUTSIDE the store's
// undo/redo stack (e.g. Documents' attachment deletes, where the binary blob
// in IndexedDB is gone for good once removed). Pattern: apply the reversible
// part immediately, show this toast, and pass the irreversible part as
// `onExpire` — it only runs once the undo window closes. Undo runs `onUndo`
// instead and the irreversible part never happens. `onExpire` is also
// flushed on pagehide so closing the tab can't leak half-deleted state.
const _pendingExpiries = new Set();
window.addEventListener('pagehide', ()=>{ for(const fn of [..._pendingExpiries]) fn(); });

export function undoToast(title, { body='', kind='ok', ms=7000, onUndo, onExpire }={}){
  const root = document.querySelector('#toasts') || document.body;
  ...
  let settled = false;
  const expire = ()=>{ if(settled) return; settled = true;
    _pendingExpiries.delete(expire); onExpire && onExpire(); dismiss(); };
  _pendingExpiries.add(expire);
  const dismiss = ()=>{ t.classList.remove('in'); setTimeout(()=>t.remove(), 260); };
  const undoBtn = el('button',{class:'btn sm', html:`${icon('undo',13)} Undo`, onclick:(e)=>{
    e.stopPropagation();
    if(settled) return; settled = true;
    _pendingExpiries.delete(expire); clearTimeout(timer);
    onUndo && onUndo(); dismiss();
  }});
  t.append(el('div',{class:'toast-actions'},[undoBtn]));
  root.append(t);
  requestAnimationFrame(()=>t.classList.add('in'));
  // Clicking the toast body (outside Undo) commits early, like dismissing.
  t.addEventListener('click', expire);
  const timer = setTimeout(expire, ms);
  return { undo: ()=>undoBtn.click(), expire };
}
