...
import { el, $$, field, modal, sheet, confirmDialog, toast, debounce, escapeHtml, fmtDate, isoDate, celebrate, anchoredPopover, copy } from '../../vendor/polecat-shell/ui.js';
...
let dashboardFilter = null;        // label of an active KPI-tile ids filter from the Dashboard, if any
...
export function renderInventory(view, ctx, params){
  ...
  // An `ids` filter (the Dashboard's KPI tiles) means "exactly these jobs, no
  // more no less" — it replaces the working filters outright rather than
  // merging, so a leftover status/type/mine filter from whatever view was
  // showing before can't silently shrink the set below what the tile counted.
  // It also carries no visible pill state of its own, so it stamps
  // `dashboardFilter` to drive a dismissible banner — otherwise landing on a
  // pre-filtered list with every pill showing "off" would look broken.
  if(window.__pendingFilter){
    const { label, ids, ...pf } = window.__pendingFilter;
    window.__pendingFilter = null;
    if(ids){ activeViewId = null; filters = { ids }; dashboardFilter = label || 'Dashboard'; }
    else { Object.assign(filters, pf); dashboardFilter = null; }
    page = 1;
  }
  ...
  function build(){
    ...
    const db = dashboardFilterBanner(); if(db) nodes.push(db);
  ...
  // The dismissible strip shown while browsing a KPI tile's ids filter from
  // the Dashboard — the only filter dimension with no pill of its own.
  function dashboardFilterBanner(){
    if(!dashboardFilter) return null;
    return el('div',{class:'callout', role:'status'},[
      el('span',{class:'ci', html:icon('filter',18)}),
      el('div',{class:'grow', style:'display:flex;align-items:center;gap:10px;flex-wrap:wrap'},[
        el('span',{text:`Filtered from the Dashboard — “${dashboardFilter}”.`}),
        el('button',{class:'btn sm ghost', text:'Clear filter', onclick:()=>{
          filters.ids = null; dashboardFilter = null; page = 1; rerender();
        }}),
      ]),
    ]);
  }
  ...
  function filterBar(){
    ...
    // On phones (≤700px, the card-list breakpoint) the wrapping pill rows are
    // replaced by this single "Filters" button that opens a bottom sheet —
    // everything after it is hidden by the media query in styles.css.
    const activeN = ['type','status','division','priority','client','letter']
      .reduce((n,k)=>n+(filters[k]?.length||0), 0)
      + (filters.rush?1:0) + (filters.overdue?1:0) + (filters.mine?1:0);
    const mob = el('button',{class:'pill fb-mobile'+(activeN?' on':''), type:'button',
      title:'Filter jobs', 'aria-haspopup':'dialog',
      html:`${icon('filter',15)}<span>Filters</span>${activeN?`<span class="filter-count">${activeN}</span>`:''}`});
    mob.addEventListener('click',()=>openFilterSheet());
    bar.append(mob);
    ...
    if((filters.status?.length||0) || restCount){
      bar.append(el('button',{class:'btn icon sm ghost', title:'Clear status, division, priority, client & letter filters', 'aria-label':'Clear these filters',
        html:icon('close',13), onclick:()=>{ filters.status=[]; filters.division=[]; filters.priority=[]; filters.client=[]; filters.letter=[]; filters.ids=null; dashboardFilter=null; page=1; rerender(); }}));
    }
    return bar;
  }
  ...
  // Shared filter-group definitions — the desktop anchored dropdowns and the
  // mobile bottom sheet render the same data.
  ...
  function openStatusFilter(anchor){ checklistDropdown(anchor, statusGroups()); }
  function openMoreFilters(anchor){ checklistDropdown(anchor, moreGroups()); }

  // Mobile filter sheet — the whole filter bar as a thumb-friendly bottom
  // sheet (ROADMAP mobile deep-pass). Same `filters` state and rerender()
  // path as the pills; the sheet floats over the view (ui.js sheet(), which
  // lives on document.body), so the list updates live behind it.
  function openFilterSheet(){
    const body = el('div',{class:'filter-sheet'});
    const jobs = Store.jobs();

    function paint(){
      body.replaceChildren();
      // Type — the primary lens, as tappable pills.
      const typeWrap = el('div',{class:'fs-group'});
      typeWrap.append(el('div',{class:'fp-group-label', text:'Type'}));
      const typePills = el('div',{class:'fs-pills'});
      Store.meta().types.forEach(t=>{
        const on = (filters.type||[]).includes(t.name);
        const p = el('button',{class:'pill'+(on?' on':''), type:'button', 'aria-pressed':on?'true':'false'});
        p.append(el('span',{class:'t-ic', html:icon(t.icon||'layers',14)}), el('span',{text:t.name}));
        p.addEventListener('click',()=>{ toggleArr('type', t.name); paint(); });
        typePills.append(p);
      });
      typeWrap.append(typePills);
      body.append(typeWrap);

      // Quick boolean toggles.
      const quick = el('div',{class:'fs-group'});
      quick.append(el('div',{class:'fp-group-label', text:'Quick'}));
      const quickPills = el('div',{class:'fs-pills'});
      [['rush','Rush','fire'],['overdue','Overdue','clock'],['mine','My jobs','star']].forEach(([key,label,ic])=>{
        const on = !!filters[key];
        const p = el('button',{class:'pill'+(on?' on':''), type:'button', 'aria-pressed':on?'true':'false',
          html:`${icon(ic,14)}<span>${label}</span>`});
        p.addEventListener('click',()=>{ filters[key]=!filters[key]; page=1; rerender(); paint(); });
        quickPills.append(p);
      });
      quick.append(quickPills);
      body.append(quick);

      // Status + the "More filters" groups, as checklist rows with live counts.
      [...statusGroups().map(g=>({ ...g, label:g.label||'Status' })), ...moreGroups()].forEach(g=>{
        if(!g.options.length) return;
        const wrap = el('div',{class:'fs-group fp-group'});
        wrap.append(el('div',{class:'fp-group-label', text:g.label}));
        g.options.forEach(opt=>{
          const on = (filters[g.key]||[]).includes(opt.value);
          const n = jobs.filter(j=>g.match(j, opt.value)).length;
          const row = el('label',{class:'fp-row'});
          const cb = el('input',{type:'checkbox', checked: on?'checked':null});
          cb.addEventListener('change',()=>{ toggleArr(g.key, opt.value); paint(); });
          row.append(cb, el('span',{style:'flex:1', text:opt.label}),
            el('span',{class:'muted', style:'font-size:11px', text:n}));
          wrap.append(row);
        });
        body.append(wrap);
      });
    ...
    const clearBtn = el('button',{class:'btn ghost', text:'Clear all', onclick:()=>{
      ['type','status','division','priority','client','letter'].forEach(k=>filters[k]=[]);
      filters.rush=false; filters.overdue=false; filters.mine=false;
      filters.ids=null; dashboardFilter=null;
      page=1; rerender(); paint();
    }});
    const doneBtn = el('button',{class:'btn primary', text:'Done', onclick:()=>hide()});
    const { hide } = sheet({ title:'Filters', icon:icon('filter'), side:'bottom', body, foot:[clearBtn, doneBtn] });
  }
