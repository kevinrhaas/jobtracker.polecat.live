...
import { el, debounce, avatarColor, initials, fmtDate, toast, celebrate, confirmDialog } from '../ui.js';
import { icon } from '../icons.js';
import { ageState, isOverdue, emptyHero } from './shared.js';
...
export function renderBoard(view, ctx, params){
  ...
  async function moveJob(id, status, { refocus=false }={}){
    ...
    if(!Store.isTransitionAllowed(job.status, status)){
      const ok = await confirmDialog({ title:'Unusual status change',
        message:`Jobs don't normally move from "${job.status}" straight to "${status}". Move it anyway?`,
        okText:'Move anyway' });
      if(!ok){ if(refocus) pendingFocusId = id; rerender(); return; }
    }
