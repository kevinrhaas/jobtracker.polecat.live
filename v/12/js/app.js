...
const TITLES = { home:'Dashboard', inventory:'Jobs', board:'Board', calendar:'Calendar', timeline:'Timeline', campaigns:'Campaigns', metrics:'Metrics', import:'Import', docs:'Documentation', admin:'Admin', settings:'Settings' };
const RENDERERS = { home:renderHome, inventory:renderInventory, board:renderBoard, calendar:renderCalendar, timeline:renderTimeline, campaigns:renderCampaigns, metrics:renderMetrics, import:renderImport, docs:renderDocs, admin:renderAdmin, settings:renderSettings };
...
function wireEvents(){
  ...
  Store.on('jobs', ()=>{ window.__notifBell?.refresh(); if(['home','inventory','board','calendar','timeline','campaigns','metrics'].includes(currentSection)) render(); });
