...
import { SCHEMA_VERSION } from '../sources/schema.js';
...
export function renderSettings(view, ctx, params={}){
  ...
  function secDataSource(){
    ...
    function openConnect(src, prefill){
      ...
      const connectBtn = el('button',{class:'btn primary', text:'Test & connect', onclick:go});
      ...
      // One connect flow at a time: a double-click must never provision or
      // adopt twice in parallel.
      let busy = false;
      async function go(){
        if(busy) return;
        busy = true; connectBtn.disabled = true; connectBtn.textContent = 'Connecting…';
        try{ await run(); }
        finally{ busy = false; connectBtn.disabled = false; connectBtn.textContent = 'Test & connect'; }
      }
      async function run(){
        ...
        if(p.state==='empty'){
          ...
          // A workspace written by a NEWER schema could be corrupted by this
          // (older) client's next write-through — block instead of guessing.
          if(p.schemaVersion && p.schemaVersion > SCHEMA_VERSION)
            return setErr(`That workspace was written by a newer JobTracker (schema v${p.schemaVersion}; this app speaks v${SCHEMA_VERSION}). Reload this app to update it, then connect.`);
          const n=(p.tables.find(x=>x.name==='jobs')||{}).count||0;
          const ok = await confirmAdopt(n);
          ...
          const list = p.tables.slice(0,6).map(t=>`${t.name} (${t.count} row${t.count===1?'':'s'})`).join(', ');
          setErr(`That database already contains other tables — ${list}${p.tables.length>6?', …':''}. Use a fresh, empty database for JobTracker.`);
        }
      }

      // The adopt confirmation, with a one-tap local backup: replacing the
      // browser copy is the one step of this flow you can't undo, so make
      // "keep a copy first" a first-class button rather than a buried tip.
      function confirmAdopt(n){
        return new Promise(resolve=>{
          let done=false;
          const finish = v=>{ if(done) return; done=true; resolve(v); m.hide(); };
          const m = modal({ title:'Load this workspace?', icon:icon('db'),
            body: el('div',{},[
              el('p',{text:`This database already has a JobTracker workspace (${n} job${n===1?'':'s'}). Connecting will replace what's in this browser with it, then keep the two in sync. Your current local data isn't uploaded.`}),
              el('p',{class:'muted tiny', text:'Want a safety copy of what’s in this browser right now? Download a backup first — you can re-import it anytime from Settings → Data & privacy.'}),
            ]),
            foot:[
              ...
              el('button',{class:'btn primary', text:'Load & connect', onclick:()=>finish(true)}),
            ],
