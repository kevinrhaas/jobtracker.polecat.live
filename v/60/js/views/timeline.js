...
let showDone = false;
let statusFilter = '';
// Large-workspace guard: rows rendered before the "Show all" footer button
// takes over (a 1000+ job window would otherwise mean thousands of grid
// cells). Per-session opt-out once the user asks for everything.
const TL_MAX = 200;
...
export function renderTimeline(view, ctx, params){
  ...
  function build(){
    ...
    if(!dated.length){
      const e = emptyHero('timeline', 'Nothing to plot yet',
        'Give a job a Date In or Due Date in the Details tab and it will appear here as a bar.');
    ...
    let rendered = 0, truncated = 0;
    for(const s of statuses){
      const list = byStatus.get(s.name);
      if(!list.length) continue;
      // Cap already hit before this group — skip its label too, counting all
      // of its rows toward the footer's "show all" total.
      if(!showAllRows && rendered >= TL_MAX){ truncated += list.length; continue; }
      ...
      row++;
      for(const r of list){
        // Cap the initially rendered rows on very large workspaces — the
        // footer button below the grid renders the rest on demand (never a
        // silent cut: it states exactly how many are hidden).
        if(!showAllRows && rendered >= TL_MAX){ truncated++; continue; }
    ...
    if(truncated>0){
      // rendered+truncated, NOT rows.length — rows also holds jobs the
      // status filters (e.g. hidden Done groups) keep off the grid entirely.
      wrap.append(el('div',{class:'cal-foot'}, [
        el('button',{class:'btn sm', html:`${icon('chevronDown',14)} Show all ${rendered+truncated} jobs (${truncated} more)`,
          onclick:()=>{ showAllRows = true; rerender(); }}),
      ]));
    }
