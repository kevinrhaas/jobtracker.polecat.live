...
import { humanSize, attIcon, emptyBlock, attachmentHistoryNode, findSimilarJobs, confirmStatusChange, undoToast } from './shared.js';
...
export function openJob(id, ctx={}){
  ...
  function attachmentsTab(){
    ...
    function attachRow(a){
      ...
      // Undo over confirmation: metadata comes off now (restorable verbatim);
      // the IndexedDB blob is only deleted once the undo window closes.
      rm.onclick = ()=>{
        const snap = { ...a };
        Store.removeAttachment(id, a.id); job=Store.job(id); renderList();
        undoToast('Attachment removed', { body:a.name,
          onUndo: ()=>{ Store.addAttachment(id, snap); job=Store.job(id); renderList(); },
