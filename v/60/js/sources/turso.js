// -----------------------------------------------------------------------
// sources/turso.js — Turso (libSQL / SQLite over HTTP). Ported from the
// polecat manager — the reference REMOTE adapter.
//
// Turso exposes a plain HTTP endpoint (`/v2/pipeline`) that accepts arbitrary
// SQL — DDL included — authenticated with a bearer token, so the whole flow
// works from a static browser page with no server and no build step:
...
//
// All writes go through one batched pipeline request, so a full workspace push
...
export const tursoSource = {
  id:'turso',
  label:'Turso',
  blurb:'A shared SQLite database over HTTP. Creates every table itself from the browser — the smoothest "connect to an empty database and go" experience. Free tier is generous.',
  icon:'db',
  accent:'#4ff8b0',
  ...
  // Cheap concurrency probe: who wrote last, and at what revision? sync.js
  // compares this against the revision it last saw — a mismatch by another
  // writer means pushing our full snapshot would clobber their changes
  // (last-write-wins), so it surfaces a conflict instead of pushing.
  async head(cfg){
    try{
      const { rows } = await query(cfg, `SELECT key, value FROM "${META_TABLE}" WHERE key IN ('rev','writer')`);
      const m = Object.fromEntries(rows);
      return { rev: m.rev!=null ? Number(m.rev) : null, writer: m.writer || null };
    }catch{ return { rev:null, writer:null }; }
  },
  ...
  async load(cfg){
    ...
    try{
      ...
      rows.forEach(([k,v])=>{
        if(k==='settings'){  try{ snap.settings  = JSON.parse(v); }catch{} }
        if(k==='meta'){      try{ snap.meta       = JSON.parse(v); }catch{} }
        if(k==='workspace'){ try{ snap.workspace  = JSON.parse(v); }catch{} }
        if(k==='rev')        snap.syncRev = Number(v);
        if(k==='writer')     snap.syncWriter = v;
      });
    }catch{}
    return snap;
  },

  // Full write-through: replace every table's contents with the snapshot in one
  // batched pipeline. Simple + robust for a workspace of hundreds of rows —
  // DELETE-then-INSERT inside the same batch is atomic on Turso.
  async save(cfg, snapshot, sync){
    try{
      ...
      metaRows(snapshot).forEach(m=>stmts.push({ sql:`INSERT OR REPLACE INTO "${META_TABLE}"(key,value) VALUES(?,?)`, args:[arg(m.key), arg(m.value)] }));
      // Revision stamp (see head()) — written in the same atomic batch.
      if(sync && sync.rev!=null){
        stmts.push({ sql:`INSERT OR REPLACE INTO "${META_TABLE}"(key,value) VALUES('rev',?)`, args:[arg(String(sync.rev))] });
        stmts.push({ sql:`INSERT OR REPLACE INTO "${META_TABLE}"(key,value) VALUES('writer',?)`, args:[arg(String(sync.writer||''))] });
      }
