...
export const Store = new (class extends Emitter{
  ...
  // 'change' too — sync.js's write-through mirror listens for it, and a
  // settings-only edit must reach the shared database like any other change.
  setSetting(key, val){ this.data.config.settings[key]=val; this._persist(); this.emit('settings', key, val); this.emit('change'); }
  ...
  updateJob(id, patch, actor='', opts={}){
    ...
    // Job numbers may intentionally repeat: a job's letter (A/B/C…) distinguishes
    // the separate pieces and revisions that share a number. Auto-assignment
    // (nextJobNumber) still picks an unused number, but manual entry is
    // deliberately unrestricted — duplicates are allowed.
  ...
  // Reorder a saved view up (-1) or down (+1) in the list — drives the order of
  // the view pills and dashboard tiles.
  moveView(id, dir){
    const a = this.data.views; const i = a.findIndex(v=>v.id===id);
  ...
  // ---- portable snapshot (pluggable data-source sync) ------------------
  // The adapter-neutral form of the whole workspace — entity tables as row
  // lists plus app-level singletons — matching sources/schema.js's shape, so a
  // DataSource adapter (Turso, …) never has to import the Store. See sync.js.
  snapshot(){
    const d = this.data;
    return {
      app:'jobtracker', schemaVersion: SCHEMA,
      tables:{
        jobs:       Object.values(d.jobs||{}),
        savedViews: (d.views||[]).slice(),
        campaigns:  (d.campaigns||[]).slice(),
        dashboards: (d.customDashboards||[]).slice(),
      },
      meta: d.meta || {},
      settings: (d.config && d.config.settings) || {},
      workspace: {
  ...
  // working copy. Rebuilds each table, runs the normal forward migration to
  // fill any gaps, clears undo (undo across a source switch is meaningless),
  // persists, and fires broad events so open views can re-render.
  replaceAll(snapshot){
    const t  = (snapshot && snapshot.tables)    || {};
    const ws = (snapshot && snapshot.workspace) || {};
    const d = this._fresh();
    d.jobs = {}; (t.jobs||[]).forEach(j=>{ if(j&&j.id) d.jobs[j.id]=j; });
    if(t.savedViews) d.views = t.savedViews.slice();
    if(t.campaigns)  d.campaigns = t.campaigns.slice();
    if(t.dashboards) d.customDashboards = t.dashboards.slice();
    if(snapshot && snapshot.meta && Object.keys(snapshot.meta).length) d.meta = snapshot.meta;
    d.favorites = ws.favorites || [];
    d.recents   = ws.recents   || [];
    d.audit     = ws.audit     || [];
    if(ws.defaultViewId    !== undefined) d.defaultViewId = ws.defaultViewId;
    if(ws.nextJobNumber)                  d.nextJobNumber = ws.nextJobNumber;
    if(ws.activeDashboardId !== undefined) d.activeDashboardId = ws.activeDashboardId;
    d.config = { ...this._freshConfig(), ...(ws.config||{}),
      settings:{ ...this._freshConfig().settings, ...((snapshot && snapshot.settings) || {}) } };
    d.schemaVersion = SCHEMA;
    this.data = this._migrate(d);
    this._undo = []; this._redo = [];
    this._persist();
    this.emit('jobs'); this.emit('meta'); this.emit('views'); this.emit('settings'); this.emit('change');
  }
