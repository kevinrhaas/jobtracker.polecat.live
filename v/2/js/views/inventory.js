...
export function renderInventory(view, ctx, params){
  ...
  function filterBar(){
    ...
    // Compact "More" popover for Type / Division / Priority.
    const more = (filters.type?.length||0) + (filters.division?.length||0) + (filters.priority?.length||0);
    bar.append(el('button',{class:'pill'+(more?' on':''), type:'button', title:'Type, division & priority filters',
      html:`${icon('filter',14)}<span>More</span>${more?`<span class="filter-count">${more}</span>`:''}`,
  ...
  function openMoreFilters(){
    ...
    const groups = [
      ['Type',     'type',     Store.meta().types.map(t=>t.name)],
      ['Division', 'division', Store.meta().divisions],
      ['Priority', 'priority', Store.meta().priorities],
    ];
    const body = groups.map(([label,key,vals])=>{
      const wrap = el('div',{class:'field'});
      wrap.append(el('label',{text:label}));
      const pills = el('div',{class:'filterbar'});
      vals.forEach(v=>{
        const on = local[key].includes(v);
        const p = el('button',{class:'pill'+(on?' on':''), type:'button', text:v});
        p.addEventListener('click',()=>{ const i=local[key].indexOf(v); if(i>=0) local[key].splice(i,1); else local[key].push(v); p.classList.toggle('on'); });
        pills.append(p);
      });
      wrap.append(pills);
      return wrap;
    });
    const { hide } = modal({ title:'More filters', icon:icon('filter'), body,
      foot:[
        el('button',{class:'btn', text:'Reset', onclick:()=>{ filters.type=[]; filters.division=[]; filters.priority=[]; hide(); page=1; rerender(); }}),
        el('button',{class:'btn primary', text:'Apply', onclick:()=>{ filters.type=local.type; filters.division=local.division; filters.priority=local.priority; hide(); page=1; rerender(); }}),
      ]});
  }
