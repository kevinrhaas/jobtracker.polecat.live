...
export function renderReports(view, ctx, params){
  ...
  if(!jobs.length){
    const e = emptyHero('reports', 'No report to generate yet',
      'Once you have some jobs, pick a period here to get a shareable summary of what shipped — breakdowns, trend, and one-click export.');
  ...
  // When embedded under the Metrics → Report tab, skip our own title (the
  // Metrics header already labels the page) and go straight to the controls.
  if(!(params && params.embedded)){
    view.append(sectionHead('Reports', 'Pick a period to see what shipped, then export or copy the summary.'));
  }
  ...
  view.append(el('div',{class:'print-footer',
    text:`JobTracker — printed ${new Date().toLocaleString('en-US',{timeZone:CT})} CT · ${rangeText}`}));
