...
export function buildRail(rail, { onNav, isAdmin=false }){
  ...
  const brand=el('button',{class:'rail-brand', title:'JobTracker — dashboard',
    html:`<span class="rail-logo">${icon('rocket',22)}</span><span class="bt"><b>JobTracker</b><small>Agency</small></span>`,
    onclick:()=>onNav('home')});
  rail.append(brand);
  ...
  SECTIONS.forEach(s=>{
    ...
    // Optional sections (Board, Campaigns) are hidden until turned on in
    // Settings → Sections — off by default to keep the nav focused.
    if(s.pref && !Store.settings()[s.pref]) return;
