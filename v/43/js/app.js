...
function buildTopbar(){
  ...
  bar.append(menuBtn, topTitle, localModeChip(), el('span',{class:'sp'}));
  ...
  bar.append(searchBtn, undoBtn, redoBtn, quickAddBtn, notifBtn, wnBtn, themeBtn, newBtn);
...
function localModeChip(){
  const chip = el('button',{class:'local-badge', type:'button', title:'Local mode — your data is stored only in this browser. Click for details.',
    html:`${icon('warn',13)}<span class="lbl">Local mode</span>`});
  chip.onclick = ()=>{
    const { hide } = modal({ title:'You’re in Local mode', icon:icon('warn'),
      body:[
        ...
        el('p',{class:'muted', text:'A shared, always-saved mode with sign-in is on the way — you’ll be able to connect this workspace to a durable database so the whole team sees the same jobs.'}),
      ],
      foot:[
        el('button',{class:'btn', text:'Back up now', onclick:()=>{ hide(); go('settings',{ section:'data' }); }}),
