...
import { humanSize, attIcon, emptyBlock, emptyHero, attachmentHistoryNode, undoToast } from './shared.js';
...
export function renderDocuments(view, ctx){
  ...
  function build(){
    ...
    if(!all.length){
      nodes.push(emptyHero('documents', 'No attachments yet', 'Files you attach to jobs will show up here, all in one place.'));
    } else if(!rows.length){
  ...
  // Undo over confirmation: the metadata comes off the jobs immediately (and
  // can be put back verbatim — addAttachment keeps a provided id/ts/version),
  // while the IndexedDB blobs are only deleted once the undo window closes.
  function bulkDelete(rows){
    const targets = rows.filter(a=>selection.has(a.id)).map(a=>({ ...a }));
    const n = targets.length;
    if(!n) return;
    targets.forEach(a=>Store.removeAttachment(a.jobId, a.id));
    selection.clear();
    rerender();
    undoToast(`Deleted ${n} file${n===1?'':'s'}`, {
      body:'The files come back if you undo.',
      // strip the view-only fields allAttachments() layered on before restoring
      onUndo: ()=>{ targets.forEach(a=>{ const { jobId, jobName, jobNumber, ...meta } = a; Store.addAttachment(jobId, meta); }); rerender(); },
      onExpire: ()=>targets.forEach(a=>deleteBlob(a.id)),
    });
  }
  ...
  function row(a){
    ...
    rm.onclick = ()=>{
      const snap = { ...a };
      Store.removeAttachment(a.jobId, a.id); rerender();
      undoToast('File removed', { body: a.name,
        onUndo: ()=>{ const { jobId, jobName, jobNumber, ...meta } = snap; Store.addAttachment(jobId, meta); rerender(); },
        onExpire: ()=>deleteBlob(snap.id),
      });
    };
    ...
    return el('div',{class:'attach-row'},[
      ...
      el('div',{style:'flex:1;min-width:0'},[
        ...
        el('div',{class:'muted tiny'},[
          el('span',{text:`${humanSize(a.size)} · v${a.version||1} · ${fmtDateTime(a.ts)}${a.mock?' · metadata only':''} · `}),
          jobLink,
        ]),
        tagsRow,
      ]),
  ...
  function showHistory(a){
    ...
    const body = attachmentHistoryNode(a, {
      onDownload: async (v)=>{
        ...
        if(!blob){ toast('File not available',{kind:'err', body:"This browser no longer has this version's bytes."}); return; }
